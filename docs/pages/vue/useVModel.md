...
在开发中，我们经常会遇到子组件修改父组件内容，但又因为单向数据流，使得写法过于麻烦。今天就记录一个快捷优雅的方式。 因为我们使用的功能是多次切重复的，所有我们封装一个 hooks。

:::tip 🔔
它可以同时处理 原始值（string/number/boolean 等）和 对象/数组 类型，支持通过属性修改对象的方式实现 v-model 的双向绑定。
:::
